...
namespace Crawler.DataService
{
    public class DBInit
    {
        ...
        /// <summary>
        /// 谨慎使用  会全部删除数据库并重新创建！TravelNotesList
        /// </summary>
        public void InitTravelNotesListTable()
        {
            #region Delete
            try
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("DROP TABLE [dbo].[TravelNotesList];");
                SqlHelper.ExecuteNonQuery(sb.ToString());
            }
            catch (Exception ex)
            {
                if (ex.Message.Contains("因为它不存在，或者您没有所需的权限。"))
                {
                    logger.Warn("初始化数据库TravelNotesList删除的时候，原表不存在");
                }
                else
                {
                    logger.Error("初始化数据库TravelNotesList失败", ex);
                    throw ex;
                }
            }
            #endregion Delete
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat(@"CREATE TABLE [dbo].[TravelNotesList](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[Title] [nvarchar](100) NULL,
	[OverView] [nvarchar](500) NULL,
	[Url] [nvarchar](200) NULL,
	[ConFrom] [nvarchar](20) NULL,
	[CityCode] [int] NULL,
...
(
	[ID] ASC
            ...
            {
                logger.Error("TravelNotesList创建异常", ex);
                throw ex;
            }
            #endregion Create
        }
