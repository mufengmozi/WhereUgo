...
namespace Crawler
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                //测试DownloadHtml
                string html = HttpHelper.DownloadHtml(@"http://www.mafengwo.cn/yj/10065/1-0-1.html", Encoding.UTF8);



                //测试获取分类页
                string html1 = HttpHelper.DownloadCategory("http://list.jd.com/list.html?cat=737%2C794%2C798&go=0");
                ...
                TravelNodesListRepository Travel = new TravelNodesListRepository();
                Console.WriteLine("请输入Y/N进行类别表初始化确认！ Y 删除TravelNodes表然后重新创建，然后抓取类型数据，N（或者其他）跳过");
                ...
                if (input.Equals("Y", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    dbInit.InitTravelNotesListTable();
                    List<TravelNotesList> TravelList = TravelNodesSearch.Crawler("http://www.mafengwo.cn/yj/10065/1-0-1.html");
                    Travel.Save(TravelList);
                    Console.WriteLine("类型数据初始化完成，共抓取类别{0}个", TravelList.Count);
                }
                else
                {
